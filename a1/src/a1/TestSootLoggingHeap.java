		...
		if(!b.getMethod().getDeclaringClass().getName().equals("a1.Log"))
		{
		    ...
		    while(it.hasNext()){
		    	...
		    	if (stmt.containsFieldRef()) {
		    		//I believe this is correct, however, I couldn't find a way to check if its a write or read statement
		    		...
		    		if(stmt.getFieldRef().getField().isStatic()) {
		    			...
		    			Log.logFieldAcc(Thread.currentThread(), field.getName(), true, false);
		    			
		    		}
		    		else {
		    			Log.logFieldAcc(Thread.currentThread(), field.getName(), false, false);
